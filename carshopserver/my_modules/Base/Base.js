...
Base.prototype.data = function(data){
    ...
    let update_str = ""
    for( key in data){
        field_str += "`"+key+"`, "//用在插入语句中的插入内容
        values_str +=  '"'+data[key]+'", '//用在插入语句中的value值
        update_str += "`"+key+"`='"+data[key]+"', "///用于更新的数据
    }
...
Base.prototype.insert = function(fun){
    // insert into news (id, title, content) values (123, '小明', '1231231')
